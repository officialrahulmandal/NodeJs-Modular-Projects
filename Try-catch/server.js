...
app.post('/messages', async (req, res) => {
    // try to save the message to the DB if not able to save that it will throw error
    try {
        // getting message from request body
        var message = new Message(req.body)

        //  saving the message to the DB
        var savedMessage = await message.save()

        // prompt this message over the console
        console.log('saved')

        // find one of the censored word
        var censored = await Message.findOne({ message: 'badword' })

        // if censored word is found in the message remove that message from the DB
        if (censored)
      ...
      // catch the errors and print that over the console
        res.sendStatus(500)
